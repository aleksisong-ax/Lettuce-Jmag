...
// Initials for the avatar circle (e.g. "WoodCraft Admin" -> "WA")
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="relative">
          ...
          <div id="notif-dropdown" class="hidden absolute right-0 mt-2 w-96 max-w-[90vw] bg-white rounded-2xl border border-gray-100 shadow-lg z-50 overflow-hidden">
            <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              ...
              <?php if ($unreadNotificationCount > 0): ?>
                <form method="POST" action="notifications-mark-all.php">
                  ...
                  <button type="submit" class="text-[12px] font-medium text-[#6B4226] hover:underline">Mark all as read</button>
                </form>
              <?php endif; ?>
            </div>
            <div class="max-h-96 overflow-y-auto divide-y divide-gray-50">
              <?php if (empty($recentNotifications)): ?>
                <p class="text-center text-sm text-gray-400 py-8">No notifications yet.</p>
              <?php else: foreach ($recentNotifications as $n): ?>
                <a href="notification-open.php?id=<?= (int)$n['id'] ?>" class="block px-4 py-3 hover:bg-gray-50 transition-colors <?= !$n['is_read'] ? 'bg-orange-50/60' : '' ?>">
                  <div class="flex items-start gap-2">
                    <?php if (!$n['is_read']): ?>
                      <span class="w-2 h-2 rounded-full bg-[#B5702E] mt-1.5 flex-shrink-0"></span>
                    <?php else: ?>
                      <span class="w-2 h-2 flex-shrink-0"></span>
                    <?php endif; ?>
                    <div class="min-w-0 flex-1">
                      <p class="text-[13px] font-semibold text-gray-900 truncate"><?= htmlspecialchars($n['title']) ?></p>
                      <p class="text-[12px] text-gray-500 truncate"><?= htmlspecialchars(strtok($n['message'], "\n")) ?></p>
            ...
            <a href="notifications.php" class="block text-center text-[12px] font-medium text-[#6B4226] hover:underline py-2.5 border-t border-gray-100">View all notifications</a>
          </div>
        </div>

        <a href="admin-profile.php" class="flex items-center gap-2.5 pl-3 border-l border-gray-200 hover:opacity-80 transition-opacity">
          <div class="w-9 h-9 rounded-full bg-[#6B4226] text-white text-xs font-semibold flex items-center justify-center"><?= htmlspecialchars($initials) ?></div>
          <div class="leading-tight text-left">
            <p class="text-sm font-semibold text-gray-900"><?= htmlspecialchars($adminName) ?></p>
            <p class="text-[11px] text-gray-400"><?= htmlspecialchars($adminRole) ?></p>
          </div>
        </a>
      </div>
    </header>
