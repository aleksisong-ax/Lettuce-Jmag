...
$categoryColors = [
    'Warranty' => 'bg-[#F1E4D1] text-[#8B5E34]',
...
<head>
  ...
  <title>FAQ Management | WoodCraft Admin</title>
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <main class="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h3 class="text-sm font-semibold text-gray-900 mb-4"><?= $editingFaq ? 'Edit FAQ' : 'Add a New FAQ' ?></h3>
          <form method="POST" class="space-y-4">
            <?php if ($editingFaq): ?>
              <input type="hidden" name="faq_id" value="<?= $editingFaq['id'] ?>" />
            <?php endif; ?>
            <div>
              <label class="block text-sm font-medium text-gray-800 mb-2">Question</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-800 mb-2">Answer</label>
              <textarea name="answer" rows="3" required placeholder="Write the answer customers will see..."
                        class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#6B4226]/40 focus:border-[#6B4226] transition-colors"><?= htmlspecialchars($editingFaq['answer'] ?? '') ?></textarea>
            </div>
            <div class="max-w-xs">
              <label class="block text-sm font-medium text-gray-800 mb-2">Category</label>
              <select name="category" class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#6B4226]/40 focus:border-[#6B4226] transition-colors">
            ...
            <div class="flex items-center gap-3">
              <button type="submit" name="save_faq" value="1" class="px-6 py-3 rounded-full bg-[#6B4226] text-white text-sm font-medium hover:bg-[#59341C] transition-colors"><?= $editingFaq ? 'Save Changes' : 'Add FAQ' ?></button>
              <?php if ($editingFaq): ?>
                <a href="admin-faq.php" class="px-6 py-3 rounded-full border border-gray-300 text-gray-800 text-sm font-medium hover:bg-gray-100 transition-colors">Cancel</a>
              <?php endif; ?>
            </div>
          </form>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <h3 class="text-sm font-semibold text-gray-900">All FAQs</h3>
            <span class="text-[12px] text-gray-400"><?= $totalFaqs ?> total</span>
          </div>
          <div class="overflow-x-auto">
            <table class="w-full">
              ...
              <tbody>
                <?php if (empty($faqs)): ?>
                  <tr><td colspan="4" class="py-10 px-4 text-center text-sm text-gray-400">No FAQs yet — add your first one above.</td></tr>
                <?php else: foreach ($faqs as $f): ?>
                  <tr class="border-b border-gray-100 last:border-0">
                    <td class="py-4 px-4 text-[13px] font-medium text-gray-900 max-w-xs"><?= htmlspecialchars($f['question']) ?></td>
                    ...
                    <td class="py-4 px-4 text-[13px] text-gray-500"><?= date('M j, Y', strtotime($f['updated_at'])) ?></td>
                    <td class="py-4 px-4">
                      <div class="flex gap-2">
                        <a href="admin-faq.php?edit=<?= $f['id'] ?>" class="text-[11px] font-medium border border-gray-200 rounded-full px-3 py-1 text-gray-600 hover:bg-gray-50 transition-colors">Edit</a>
