...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' && isset($_SESSION['pending_warranty'])) {
    $pending = $_SESSION['pending_warranty'];
    ...
    $formData['warranty_issue'] = $pending['warranty_issue'] ?? '';
    $formData['defect_description'] = $pending['defect_description'] ?? '';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['confirm_submit']) && $_POST['confirm_submit'] === '1') {
        $submittedData = $_SESSION['pending_warranty'] ?? null;
        
        if ($submittedData) {
            $product_name = $submittedData['product_name'];
            $order_number = $submittedData['order_number'];
            $purchase_date = $submittedData['purchase_date'];
            $warranty_issue = $submittedData['warranty_issue'];
            $defect_description = $submittedData['defect_description'];
            $proof_path = $submittedData['proof_path'] ?? null;
            $damage_path = $submittedData['damage_path'] ?? null;

            try {
                $stmt = $conn->prepare("
                    INSERT INTO warranty_requests (user_id, product_name, order_number, purchase_date, warranty_issue, defect_description, proof_of_purchase_path, damage_photo_path, status)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')
                ");
                ...
                if ($success) {
                    ...
                    unset($_SESSION['pending_warranty']);
                    header("Location: my-profile.php?warranty=1");
                    exit();
                } else {
                    $message = "Something went wrong submitting your claim. Please try again.";
                    $messageType = "error";
                }
        ...
        $warranty_issue = trim($_POST['warranty_issue'] ?? '');
        ...
        $formData['warranty_issue'] = $warranty_issue;
        $formData['defect_description'] = $defect_description;
        ...
        if (empty($product_name) || empty($order_number) || empty($warranty_issue) || empty($defect_description) || $purchase_date === '') {
            ...
            if (!$proofUpload['ok']) {
                ...
                if (!$damagePhotoUpload['ok']) {
                    ...
                    $submittedData = $_SESSION['pending_warranty'];
                }
            }
        }
    }
          ...
          <div class="modal-content bg-white rounded-3xl shadow-2xl p-8">
            <div class="flex items-start justify-between mb-6">
              <div>
                ...
                <h2 class="font-serif text-2xl font-semibold text-gray-900">Review Your Warranty Claim</h2>
                <p class="text-gray-500 text-sm mt-1">Please verify all details before submitting.</p>
              </div>
              <a href="warranty-request.php" class="text-gray-400 hover:text-gray-600 transition-colors text-2xl leading-none">✕</a>
            </div>
            ...
            <div class="space-y-4">
              ...
              <div class="confirm-field">
                <span class="confirm-label">Defect Description</span>
                <p class="confirm-value whitespace-pre-line"><?= nl2br(htmlspecialchars($submittedData['defect_description'])) ?></p>
              </div>
            ...
            <div class="flex flex-wrap gap-3 mt-6 pt-6 border-t border-gray-100">
              ...
              <a href="warranty-request.php" class="px-6 py-3 rounded-full border border-gray-300 text-gray-700 text-sm font-medium hover:bg-gray-50 transition-colors">
                ← Edit Details
              </a>
            </div>
            
            <p class="text-[12px] text-gray-400 mt-4">
              By submitting, you agree to our warranty terms. Our team will review your claim within 1-2 business days.
            </p>
          </div>
        </div>
      <?php endif; ?>

      <form class="space-y-5 mt-6" method="POST" enctype="multipart/form-data" novalidate id="warrantyForm">
          <div>
          ...
          <div>
            ...
            <select name="warranty_issue" required class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#6B4226]/40 focus:border-[#6B4226] transition-colors">
              ...
              <?php foreach ($warrantyIssues as $issue): ?>
                <option value="<?= htmlspecialchars($issue) ?>" <?= ($formData['warranty_issue'] === $issue) ? 'selected' : '' ?>>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-800 mb-2">Describe the Defect</label>
            <textarea rows="4" name="defect_description" required placeholder="Tell us what's wrong..." class="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#6B4226]/40 focus:border-[#6B4226] transition-colors"><?= htmlspecialchars($formData['defect_description']) ?></textarea>
          </div>
          
          <div>
            <label class="block text-sm font-medium text-gray-800 mb-2">Proof of Purchase</label>
