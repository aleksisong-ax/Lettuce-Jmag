    ...
    if (empty($email) || empty($password)) {
        ...
        if ($stmt->rowCount() == 1) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                // The customer's own Live Chat conversation
                // (includes/chat-session.php) is loaded fresh via their
                // permanent account-based key and is never merged with
                // whatever was chatted as a guest (that lived only in
                // the browser tab's sessionStorage, not the session).
...
<head>
  ...
  <title>Login | WoodCraft Care</title>
...
<body class="bg-[#F3F0E4] text-gray-900 min-h-screen flex flex-col">
  ...
  <main class="flex-1 flex items-center justify-center px-6 py-16">
    <div class="w-full max-w-md bg-white rounded-3xl shadow-sm border border-gray-100 p-9">
      <span class="inline-block text-[11px] font-semibold tracking-wide text-gray-500 bg-gray-100 rounded-full px-3 py-1 mb-5">LOGIN</span>
      <h1 class="font-serif text-3xl font-semibold text-gray-900 mb-2">Welcome back</h1>
      <p class="text-gray-500 text-sm mb-8">Sign in to manage your support tickets and requests.</p>
...
<?php endif; ?>
      ...
      <form class="space-y-5" method="POST">
        ...
        <button type="submit" class="w-full rounded-full bg-[#6B4226] text-white text-sm font-medium py-3.5 hover:bg-[#59341C] transition-colors">Sign In</button>

        <div class="flex items-center justify-between pt-1">
          <a href="forgot-password.php" class="text-sm text-[#6B4226] hover:text-[#59341C] transition-colors">Forgot password?</a>
          <a href="register.php" class="text-sm text-[#6B4226] hover:text-[#59341C] transition-colors">Create account →</a>
        </div>
      </form>
    </div>
  </main>
